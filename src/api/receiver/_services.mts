...
import { Row_Receiver } from "jm-castle-warehouse-types";
import {
...
import { TableReceiver } from "../../persistence/maria-db/tables/Receiver.mjs";
...
allServices.push({
  ...
  name: "Select paginated receivers.",
  handler: [
    async (req, res) => {
      try {
        ...
        if (
          ...
          withDefaultPersistence(res, async (persistence) => {
            const response =
              await persistence.tables.pagination.selectPage<Row_Receiver>(
                TableReceiver,
                pageNumber,
                pageSizeNumber
              );
