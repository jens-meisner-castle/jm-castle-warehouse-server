...
import { TableImageContent } from "../../persistence/maria-db/tables/ImageContent.mjs";
...
allServices.push({
  ...
  name: "Select paginated image content rows.",
  handler: [
    async (req, res) => {
      try {
        ...
        if (
          ...
          withDefaultPersistence(res, async (persistence) => {
            const response =
              await persistence.tables.pagination.selectPage<Row_ImageContent>(
                TableImageContent,
                pageNumber,
                pageSizeNumber
              );
